# Literacy Diagnostic Scoring Logic (Grades 4–9)
...
def assess_literacy_mastery_by_strand(scores_dict):
    """
    Assigns mastery levels based on percent scores per literacy strand.
    Levels:
    - Mastered: 90–100
    - In Progress: 70–89
    - Not Yet Mastered: < 70
...
def export_results_to_json(results, filename="literacy_results.json"):
    import json
    with open(filename, "w") as f:
        json.dump(results, f, indent=4)

def export_results_to_csv(results, filename="literacy_results.csv"):
    import csv
...
# Example input for a student across literacy strands
student_scores = {
    "Reading Comprehension": 88,
    "Vocabulary Acquisition": 92,
    "Writing Mechanics": 75,
    "Text Structure & Purpose": 66,
    "Literary Analysis": 94,
    "Grammar & Usage": 58
}

# Run diagnostic scoring
results = assess_literacy_mastery_by_strand(student_scores)
print("Literacy Diagnostic Results:")
for strand, level in results.items():
